...
class InvestmentDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables($query)
                ->addIndexColumn()
                ->editColumn('investment', function($id){
                    return number_format($id->investment, 0, ',', '.');
                })
                ->editColumn('return', function($id){
                    return number_format($id->return, 0, ',', '.');
                })
                ->editColumn('portfolio', function($id){
                    return number_format($id->portfolio, 0, ',', '.');
                })
    ...
    public function query(Investment $model): QueryBuilder
    {
        $tahun = $this->tahun;
        $bulan = $this->bulan;
        return $model->whereYear('date',$tahun)->whereMonth('date',$bulan)->orderBy('date','DESC')->newQuery();
    }
