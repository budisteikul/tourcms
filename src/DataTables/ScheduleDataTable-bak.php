...
class ScheduleDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return datatables($query)
                ...
                ->addColumn('name', function($id){
                    ...
                    return '<a href="#" onClick="SHOW(\''.$id->shoppingcart->id.'\'); return false;">'. $name .'</a> '. $people;
                })
                ->addColumn('single_view', function($id){
                    $name = '';
                    foreach($id->shoppingcart->shoppingcart_questions as $question)
                    {
                        $name .= $question->answer .' ';
                    ...
                    $date = GeneralHelper::dateFormat($id->date,10);

                    $channel = $id->shoppingcart->booking_channel;

                    $tour = $id->title;
                    return '
                    <div class="row col-12">
                    <div class="col-6">
                        <a href="#" onClick="SHOW(\''.$id->shoppingcart->id.'\'); return false;">'. $name .'</a> '. $people .' pax
                    </div>
                ...
                ->addColumn('action', function ($id) {
                    ...
                    if($contact->phoneNumber!="")
                    {
                        ...
                        $button_wa = '<a target="_blank" id="btn-wa" href="/cms/contact/'.$wa.'/edit" class="btn btn-sm btn-primary text-white"><i class="fab fa-whatsapp"></i> Whatsapp</a>'; 
                    }
                return '
  ...
  <div class="dropdown-menu">
    ...
    <a id="btn-edit"  href="#" onClick="EDIT(\''.$id->shoppingcart->id.'\'); return false;" class="dropdown-item">Reschedule</a>
    ...
    public function query(ShoppingcartProduct $model): QueryBuilder
    {
        $model = $model->with(['shoppingcart' => function ($query) {
                 ...
                 })->whereDate('date', '>=', date('Y-m-d'))->whereNotNull('date')->orderBy('date')->orderBy('id')->newQuery();
